...
module.exports = function create(context) {
    ...
    class JS_FileSystemObject extends Component {
        ...
        buildpath (existing_path, new_path_part) {
            return context.vfs.BuildPath(existing_path, new_path_part);
        }

        // Copies one or more files from one location to another.
        copyfile (source, destination, overwrite_files) {

            source      = context.vfs.Resolve(source);
            destination = context.vfs.Resolve(destination);
            ...
            if (is_source_wildcard) {
                ...
                let matched_file_list = context.vfs.FindFiles(source_dirname, source_basename);
            ...
            try {
                ...
                for (let i = 0; i < copy_from_to.length; i++) {
                    context.vfs.CopyFile(copy_from_to[i].source, copy_from_to[i].dest);
                }
            }
            catch (e) {
        ...
        copyfolder (source, destination, overwrite) {
            ...
            if (context.vfs.IsWildcard(src_dirname)){
            ...
            if (context.vfs.IsWildcard(dst_basename) || context.vfs.IsWildcard(dst_dirname)) {
                this.context.exceptions.throw_invalid_fn_arg(
                    "Scripting.FileSystemObject",
            ...
            const dir_items_list = (context.vfs.IsWildcard(src_basename))
                      ? context.vfs.FindFolders(src_dirname, src_basename)
                      : [src_basename];
            ...
            for (let i = 0; i < dir_items_list.length; i++) {
                ...
                try {
                    context.vfs.CopyFolder(src_path, destination, overwrite);
                }
                catch (e) {
        ...
        createfolder (path) {
            ...
            const invalid_arg = [
                ...
                context.vfs.IsWildcard(path),
                context.vfs.IsPathIllegal(path)
            ].some(x => x === true);
            ...
            if (context.vfs.FolderExists(path)) {
                this.context.exceptions.throw_file_already_exists(
                    "Scripting.FileSystemObject",
        ...
        createtextfile (filespec, overwrite, unicode) {
            ...
            if (context.vfs.IsWildcard(filespec)) {
            ...
            if (context.vfs.PathIsRelative(filespec)) {
            ...
            if (context.vfs.FolderExists(dirname) === false) {
                this.context.exceptions.throw_path_not_found(
                    "FileSystemObject",
        ...
        deletefile (filespec, force) {
            ...
            if (context.vfs.IsWildcard(path) || context.vfs.IsFilespecIllegal(pattern)) {
                this.context.exceptions.throw_bad_filename_or_number(
                    "FileSystemObject",
            ...
            if (context.vfs.FolderExists(filespec)) return 0;

            let num_deleted = context.vfs.DeleteInFolderMatching(path, pattern);
        ...
        deletefolder (pathspec) {
            ...
            if (context.vfs.IsWildcard(path)) {
                this.context.exceptions.throw_bad_filename_or_number(
                    "FileSystemObject",
            ...
            let num_deleted = context.vfs.DeleteInFolderMatching(
        ...
        fileexists (filepath) {
            ...
            if (context.vfs.IsWildcard(filepath) || /[\\/]$/.test(filepath)) {
                return false;
            }
            ...
            return context.vfs.FileExists(filepath);
        }
        ...
        folderexists (dirpath) {
            ...
            if (context.vfs.IsWildcard(dirpath)) {
                return false;
            }

            if (context.vfs.PathIsRelative(dirpath)) {
                dirpath = win32path.join(this.context.get_env("path"), dirpath);
            }

            return context.vfs.FolderExists(dirpath);
        }
        ...
        getfileversion (filepath) {
            ...
            if (context.vfs.FileExists(filepath) === false) {
                this.context.exceptions.throw_error(
                    "FileSystemObject",
                    "Cannot find file.",
        ...
        movefile (source, destination) {
            ...
            let srcpath                  = context.vfs.Resolve(source, { sfn_to_lfn: true }),
                srcpath_parent_dir       = win32path.dirname(srcpath),
                dstpath                  = context.vfs.Resolve(destination, { sfn_to_lfn: true }),
                dstpath_trailing_pathsep = /[\\/]$/.test(destination),
                dstpath_exists           = (context.vfs.FileExists(dstpath) || context.vfs.FolderExists(dstpath));
            ...
            else if (context.vfs.IsWildcard(srcpath_parent_dir)) {
                this.context.exceptions.throw_invalid_fn_arg(
                    "FileSystemObject",
            ...
            if (context.vfs.IsFolder(srcpath)) {
                this.context.exceptions.throw_file_not_found(
                    "FileSystemObject",
            ...
            if (context.vfs.IsWildcard(srcpath)) {

                const pattern     = win32path.basename(srcpath),
                      match_files = context.vfs.FindFiles(srcpath_parent_dir, pattern);
                ...
                match_files.forEach(filename => {
                    ...
                    if (context.vfs.Exists(fullpath_dst)) {
                    ...
                    context.vfs.MoveFile(fullpath_src, fullpath_dst);
                });
                return;
            }
            ...
            if (context.vfs.FolderExists(dstpath)) {
                dstpath = win32path.join(dstpath, win32path.basename(srcpath));
            }

            if (context.vfs.FileExists(dstpath)) {
                this.context.exceptions.throw_file_already_exists(
                    "FileSystemObject",
            ...
            context.vfs.MoveFile(srcpath, dstpath);
        }
        ...
        movefolder (source, destination) {
            ...
            const srcpath = context.vfs.Resolve(source),
                  dstpath = context.vfs.Resolve(destination);

            if (context.vfs.IsWildcard(dstpath)) {
                this.context.exceptions.throw_invalid_fn_arg(
                    "FileSystemObject",
                    "Destination path must not contain wildcard chars.",
                    "The destination path contains wildcard character(s) " +
            ...
            else if (context.vfs.IsWildcard(win32path.dirname(srcpath))) {
                this.context.exceptions.throw_invalid_fn_arg(
                    "FileSystemObject",
                    "Source path may only contain wildcards as the last part.",
                    "Only the last part of the source path may contain wildcard " +
            ...
            if (context.vfs.IsWildcard(srcpath)) {
                ...
                const pattern       = win32path.basename(srcpath),
                      ...
                      match_folders = context.vfs.FindFolders(src_dirname, pattern);
                ...
                match_folders.forEach(foldername => {
                    ...
                    if (!context.vfs.Exists(fullpath_dst)) {
                        context.vfs.AddFolder(fullpath_dst);
                    }
                    else {
                        this.context.exceptions.throw_file_exists(
                            "FileSystemObject",
                    ...
                    context.vfs.MoveFolder(fullpath_src, `${fullpath_dst}\\`, false);
                });
                return;
            }

            if (context.vfs.Exists(srcpath) === false) {
                this.context.exceptions.throw_path_not_found(
                    "FileSystemObject",
            ...
            else if (context.vfs.IsFile(srcpath)) {
                this.context.exceptions.throw_path_not_found(
                    "FileSystemObject",
            ...
            if (context.vfs.Exists(win32path.dirname(dstpath)) === false) {
                this.context.exceptions.throw_path_not_found(
                    "FileSystemObject",
            ...
            else if (context.vfs.IsFile(dstpath)) {
                this.context.exceptions.throw_file_already_exists(
                    "FileSystemObject",
            ...
            context.vfs.MoveFolder(srcpath, dstpath, false);
        }
        ...
        opentextfile (filepath, iomode, create, format) {
            ...
            filepath = context.vfs.Resolve(filepath);

            if (create && context.vfs.Exists(filepath) === false) {
