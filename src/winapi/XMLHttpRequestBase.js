...
function create (context, tag, type) {
    ...
    class XMLHttpRequestBase extends Component {
	    ...
	    request.ua     = this.context.user_agent;
	    this.setrequestheader("User-Agent", request.ua);
        }
        ...
        //
        get statustext () {
	    let friendly_status = HTTPStatus[response.status];
	    ...
	    if (response.headers.hasOwnProperty(header.toLowerCase())) {
	        header_value = response.headers[header.toLowerCase()];
	    }
	    ...
	    request.body = body;
            ...
            response = this.context.get_nethook(request);
	    ...
	    let existing_hdr_idx = request.headers.findIndex(
	    ...
	    if (existing_hdr_idx > -1) {
	        request.headers[existing_hdr_idx] = hdr;
	    }
	    else {
	        request.headers.push(`${header}: ${val}`);
	    }
        }
    }
